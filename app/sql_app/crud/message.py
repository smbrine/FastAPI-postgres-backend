...
from app.sql_app.schemas import MessageContentPublicSchema, MessageCreateSchema, MessagePublicSchema
...
class MessageCRUD(BaseCRUD):
    ...
    async def add_message(self, message: MessageCreateSchema):
        ...
        new_contents = [models.MessageContent(
            id=str(uuid.uuid4()), message_id=new_message_id, content=content.content, order=content.order,
        ...
        for message_content in new_contents:
            message_content_obj = await self.MessageContentDB.add_message_content(message_content)
            if not message_content_obj:
                raise HTTPException(status_code=501)
            message_contents_obj.append(message_content_obj)

        return_contents = [MessageContentPublicSchema(
            content=message_content.content, order=message_content.order, content_type=message_content.content_type, )
            for message_content in message_contents_obj]

        return MessagePublicSchema(
            from_id=message.from_id, contents=return_contents
        )
