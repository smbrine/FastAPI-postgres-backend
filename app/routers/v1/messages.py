...
@router.post('/add', response_model=MessagePublicSchema, tags=["messages", 'add', 'post'])
async def add_message(message: MessageCreateSchema):
    return await MessageDB.add_message(message=message)


@router.get('/{message_id}', response_model=MessagePublicSchema, tags=["messages", 'get'])
async def get_message(message_id: str):
    return await MessageDB.get_message(message_id=message_id)


@router.delete('/{message_id}', tags=["messages", 'delete'])
